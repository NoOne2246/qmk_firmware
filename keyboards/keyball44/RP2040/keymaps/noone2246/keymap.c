...
#ifdef OLED_ENABLE
#    include "RP2040/keymaps/noone2246/oledkit.h"
#endif
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [_MOUSE] = LAYOUT_right_ball(
    ...
    _______       , C(KC_A)  , C(KC_S)  , _______  , C(KC_F)   , _______  ,                                        _______     , MS_BTN1     , MS_BTN3     , MS_BTN2     , _______  , _______  ,
    OSM(MOD_LSFT) , _______  , C(KC_Z)  , C(KC_X)  , C(KC_C)   , C(KC_V)  ,                                        _______     , MS_WHLL     , MS_WHLD     , MS_WHLR     , _______   , _______  , 
...
static inline int8_t clip2int8(int16_t v) {
 ...
 report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    ...
    if(abs(mouse_report.x) + abs(mouse_report.y) > 0){
        ...
        if(held_report.buttons != MOUSE_BTN2){
            if(abs(mouse_report.x) + abs(mouse_report.y) < 5)
            {
                held_report.x = 0;
                held_report.y = 0;
            }else{
